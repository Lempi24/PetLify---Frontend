...
export default defineConfig({
	plugins: [
		...
		VitePWA({
			...
			injectRegister: 'auto', // Automatyczna rejestracja SW

			// Definicja Web App Manifest (generuje manifest.webmanifest)
			manifest: {
				name: 'PetLify',
				short_name: 'PetLify',
				description:
					'Aplikacja do zgłaszania zaginięć i znalezień zwierząt, żeby szybciej łączyć właścicieli z ich pupilami.',
				theme_color: '#ce7f31', // Kolor paska adresu/tła
				display: 'standalone',
				background_color: '#ce7f31',
				start_url: '/',
				icons: allIcons.map((icon) => ({
					...icon,
					type: icon.src.endsWith('.png') ? 'image/png' : 'image/jpeg',
				})),
			},

			// Konfiguracja strategii cache'owania zasobów (Workbox)
			workbox: {
				globPatterns: ['**/*.{js,css,html,ico,png,svg}'],
			},

			devOptions: {
				// Umożliwia testowanie wtyczki w trybie development
				enabled: true,
			},
		}),
	],
