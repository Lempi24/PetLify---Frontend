...
const parsePetAge = (ageString) => {
	...
	const ageUnit =
		ageMatch[2].toLowerCase().includes('lat') ||
		ageMatch[2].toLowerCase().includes('year')
			? 'lat'
			: 'miesięcy';
...
const CreatePetProfile = ({ onClose, onSuccess, editPet = null }) => {
	...
	const validateForm = () => {
		...
		if (
			formData.petAgeValue &&
			formData.petAgeUnit === 'months' &&
			formData.petAgeValue > 11
		) {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const ageString = formData.petAgeValue
				? `${formData.petAgeValue} ${
						formData.petAgeUnit === 'months' ? 'miesięcy' : 'lat'
				  }`
				: '';
			...
			if (error.response?.data?.message) {
				...
				setErrors({
					general: `Nie udało się ${
						editPet ? 'zaktualizować' : 'utworzyć'
					} profilu`,
				});
			}
		} finally {
				...
				<form onSubmit={handleSubmit} className='space-y-6'>
						...
						<div className='flex flex-wrap gap-3 mb-3'>
							...
							{photos.length + existingPhotos.length < 5 && (
								<label className='w-20 h-20 border-2 border-dashed border-cta rounded-lg flex items-center justify-center cursor-pointer transition-colors group'>
									<span className='text-cta text-2xl group-hover:scale-180 transition-transform duration-300 ease-in-out'>
										+
									</span>
									<input
										type='file'
							...
							<div className='flex gap-0 rounded-md overflow-hidden'>
								<div className='flex-1'>
									<input
										...
										onChange={(e) =>
											handleInputChange('petAgeValue', e.target.value)
										}
										min='0'
								...
								<select
									...
									onChange={(e) =>
										handleInputChange('petAgeUnit', e.target.value)
									}
					...
					<div className='flex gap-3 justify-end pt-4'>
						...
						<button
							...
							disabled={
								loading ||
								(formData.petAgeUnit === 'months' && formData.petAgeValue > 11)
							}
					...
					>
						...
						<div className='flex items-center justify-around gap-2 mt-4 lg:mt-0'>
							{!pet.is_lost ? (
								...
								<p className='bg-secondary text-white py-2 px-4 rounded-lg text-sm'>
									Zaginiony
								</p>
							)}
							<button
						...
						>
							...
							<div className='flex flex-col items-center text-center'>
								...
								<p className='text-accent mb-4'>
									{[
										getSpeciesLabel(
											selectedPet.pet_species_type || selectedPet.pet_species
										),
				...
				<div className='fixed inset-0 backdrop-blur-2xl flex items-center justify-center z-10000'>
					<div className='bg-main p-6 rounded-lg shadow-lg flex flex-col items-center gap-4 max-w-md mx-4 border-2 border-cta'>
						...
						<div className='flex gap-4'>
							<button
								onClick={() =>
									handleDeletePetProfile(petToDelete.id, petToDelete.pet_name)
								}
