  ...
  const fetchFoundPets = async () => {
    ...
    try {
      const response = await axios.get(
        import.meta.env.VITE_BACKEND_URL + `/main-page/fetch-pets?type=found&status=pending`,
        {
          headers: {
    ...
    try {
      if (pendingAction.type === 'approve') {
        await axios.post(
          ...
          { reportId: pendingAction.petId, reportType: 'found' },
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
      ...
      setFoundPets((prev) => prev.filter((pet) => pet.id !== pendingAction.petId));
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center justify-center text-center mt-16 px-4">
        ...
        <p className="text-md text-gray-400 mb-10">Tutaj możesz akceptować lub odrzucać zgłoszenia znalezionych zwierząt.</p>
      </div>
