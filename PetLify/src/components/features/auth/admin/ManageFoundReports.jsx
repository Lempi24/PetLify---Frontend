		...
		try {
			const response = await axios.get(
				import.meta.env.VITE_BACKEND_URL +
					`/main-page/fetch-pets?type=found&status=pending`,
				{ headers: { Authorization: `Bearer ${token}` } }
			);

			console.log('fetchFoundPets response:', response.data);

			const petsWithLabels = (response.data.items || response.data || []).map(
				(pet) => {
					if (pet.pet_species) {
						pet.pet_species =
		...
		<>
			...
			<div className='flex flex-col items-center justify-center text-center mt-16 px-4'>
				...
				<p className='text-md text-gray-400 mb-10'>
					Tutaj możesz akceptować lub odrzucać zgłoszenia znalezionych zwierząt.
				</p>
			</div>
